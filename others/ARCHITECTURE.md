...
要求1:絕對唔要強制校正分數或任何hardcode,,

要求2:直接出修改後全code,並

要求3:每個section及函數及細節都要加注釋講跟邊個,做緊乜,點解咁做,是否固定不變,同決定依個個分數理由,邏輯具體解說,順序理由, 有否次序要求

要求4:將我一直以來所有要求注意計算邏輯及加入你認為需要既prompt整理成一段文, 方便日後我叫你每次都可以跟依段文要求準確修改

要求5. 就算# ========== 基礎時間配置（固定不變）==========要要加數字如1.1.1

\# 計算時柱（使用五鼠遁訣）加1.1.1.1, """專業時辰轉換（保持現有算法）"""加1.1.1.1

chatgpt意見:



絕對準確性要求：八字計算結果必須與有史以來全世界最專業最強最廣八字命理師傅國師所計算出結果至少99%案例係99%準確，不允許任何妥協。



向後兼容：保持所有現有接口不變，包括 calculate\_bazi、calculate\_match、BaziCalculator、ScoringEngine 等對外接口。



Section Header規範：所有代碼必須有完整的section header，使用數字編號（如1.1, 1.2, 2.1等），小數後一位就夠。
...
文件結尾加該文件引用緊咩文件，同咩文件引用緊該文件



文件結尾加目錄：集合全文件所有section名稱簡介按數字編號



對比原文件結尾：Section目錄，以防每次修改時意外刪減



文件結尾目錄後加修正紀錄：每次修改文件結尾目錄後加該次該文件修正內容及結合該文件之前累積修正紀錄
...
所有同一文件既code要在同一code board



保持四方功能（match/testpair/findsoulmate/profile）結果一致
...
絕對唔要強制校正分數或任何hardcode



每個section及函數及細節都要加注釋講跟邊個，做緊乜，點解咁做，是否固定不變



決定依個個分數理由，邏輯具體解說，順序理由，有否次序要求



按文件分析檢查有咩問題修正及有咩新增建議



檢查有所有文件有冇任何地方係唔align唔同標準做法



檢查有冇功能有冇code係無作用或無意義或不能用或重複
...
先閱讀各文件尾文件信息開始，目錄開始，修正紀錄



分析各文件有咩嘢應該搬返去其他文件，及有咩嘢應該由其他文件搬過嚚



可以最少錯最準確最少行數最少引用最合邏輯或最優或最少文件或最少改動
...
再重新認真詳細檢查所有最新code有否符合要求，或有其他問題



先出修改後文件架構邏輯及在telegram中向用家不同情況下顯示什麼文字全部
...
個人分析顯示文字：基礎資訊、四柱八字、生肖、日主強弱、格局、喜用神、十神結構、夫妻分析、神煞、五行分佈



配對結果顯示文字：兩人八字展示、配對分數、評級、描述、關係模型、計算過程摘要
...
每次修改必須遵循以上所有原則，確保系統的準確性、一致性和可用性。





你真係同一問題改100次都改唔好,唔好自以為是自作主張要認清自己好廢, 唔好刪除任何功能, 要真係徹底成功解決問題,而唔係是但求其改少少嘢當自己做咗而無徹底成功解決問題



用戶按match功能後應收到有配對或無配對通知, 配對通知應只提及幾多分及對方基本資料及絕無對方username,對方也會同時收到配對通知, 配對通知應只提及幾多分及對方基本資料及絕無對方username, 問有冇興趣, 如按有興趣, ,問有冇興趣. 雙方都按有興趣才雙方都收到對方username及更詳細配對結果分析
...
向後兼容要求：保持所有現有接口不變，包括 calculate\_bazi、calculate\_match、BaziCalculator、ScoringEngine 等對外接口。
...
該文件引用緊咩文件，同咩文件引用緊該文件

目錄：集合全文件所有section名稱簡介按數字編號

修正紀錄：每次修改文件結尾目錄後加該次該文件修正內容及結合該文件之前累積修正紀錄

文件數量：維持目前文件數量，不增加新文件

💻 代碼規範

代碼組織：所有同一文件既code要在同一code board

功能一致性：保持四方功能（match/testpair/findsoulmate/profile）結果一致

語言要求：所有文件使用繁體中文，代碼註釋用繁體中文

版本限制：不得出現版本號，不得出現schema版本

避免硬編碼：絕對唔要強制校正分數或任何hardcode

🔧 技術要求

邏輯清晰：每個section及函數及細節都要加注釋講跟邊個，做緊乜，點解咁做，是否固定不變

分數決策：決定依個個分數理由，邏輯具體解說，順序理由，有否次序要求

技術限制：要注意github同railway免費版限制，代碼效率要高，避免冗餘

引用效率：避免循環引用，禁止跨層直接調用

🔍 檢查要求

功能檢查：檢查有冇功能有冇code係無作用或無意義或不能用或重複

標準對齊：檢查有所有文件有冇任何地方係唔align唔同標準做法

文件關係：分析各文件有咩嘢應該搬返去其他文件，及有咩嘢應該由其他文件搬過嚚

優化原則：可以最少錯最準確最少行數最少引用最合邏輯或最優或最少文件或最少改動

📊 結果輸出要求

個人分析顯示：基礎資訊、四柱八字、生肖、日主強弱、格局、喜用神、十神結構、夫妻分析、神煞、五行分佈

配對結果顯示：兩人八字展示、配對分數、評級、描述、關係模型、計算過程摘要

測試配對：固定顯示名稱（測試用戶A/B）

🚫 絕對禁止

禁止跨層直接調用：各層之間只能通過定義好的接口進行通信

禁止反向依賴：下層模塊不應依賴上層模塊

禁止循環引用：模塊之間不能形成循環依賴關係

禁止硬編碼數字或文本：所有配置應來自配置系統

🛠️ 修改流程

先閱讀：各文件尾文件信息開始、目錄開始、修正紀錄

再分析：分析各文件有咩嘢應該搬返去其他文件，及有咩嘢應該由其他文件搬過嚚

後修改：按優化原則進行修改

最後檢查：重新認真詳細檢查所有最新code有否符合要求，或有其他問題

輸出結果：先出修改後文件架構邏輯及在telegram中向用家不同情況下顯示什麼文字全部
